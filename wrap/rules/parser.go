...
func ParseRule(line, sourceFile string) (commontypes.Rule, error) {
	raw := strings.TrimSpace(line)
	if raw == "" || strings.HasPrefix(raw, "#") {
		return commontypes.Rule{}, fmt.Errorf("empty or comment")
	}

	open := strings.Index(raw, "(")
	close := strings.LastIndex(raw, ")")
	if open < 0 || close <= open {
		return commontypes.Rule{}, fmt.Errorf("missing rule options")
	}

	header := strings.Fields(strings.TrimSpace(raw[:open]))
	if len(header) != 2 && len(header) < 7 {
		return commontypes.Rule{}, fmt.Errorf("invalid rule header")
	}
	opts := raw[open+1 : close]

	sid, err := requiredInt(optionSID, opts, "sid")
	if err != nil {
		return commontypes.Rule{}, err
	}
	gid := int64(1)
	if value, ok := optionalInt(optionGID, opts); ok {
		gid = value
	}
	rev, _ := optionalInt(optionRev, opts)

	rule := commontypes.Rule{
		SID:        sid,
		GID:        gid,
		Rev:        rev,
		Action:     header[0],
		Proto:      header[1],
		Msg:        optionalString(optionMsg, opts),
		Classtype:  optionalString(optionClasstype, opts),
		Enabled:    true,
